...
class Emacspeaker(object):
	...
	def __init__(self):
		...
		self._queue=[] # TTS queue
		self._cmdMap={
			'tts_saytext': functools.partial(speech.speak, priority=speech.SPRI_NOW), #todo: handle morpheme boundaries as described in the spec
			'q': self.q,
			'd': self.d,
			'l': self.l,
	...
	def d(self, args):
		...
		speech.speak(res, priority=speech.SPRI_NOW)
		self._queue=[] # Empty the queue, since we've already processed it.
	...
	def l(self, args):
		...
		res.extend(list(speech.getSpeechForSpelling(''.join(args))))
...
class TCPHandler(SocketServer.StreamRequestHandler):
	...
	def handle(self):
		global e
		e.parseCommand(self.rfile.readline().strip()) #Todo: we want long-lived TCP connections, creating sockets for every request is expensive


def start():
	"Starts the Emacspeak TCP server."
	server = SocketServer.TCPServer(("localhost", 6832), TCPHandler)
	serverthread=threading.Thread(target=server.serve_forever)
	serverthread.start()


e = Emacspeaker() # Todo: integrate this in a custom TCPServer class instead of using globals
